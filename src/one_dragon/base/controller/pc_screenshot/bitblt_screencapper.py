...
class BitBltScreencapper(ScreencapperBase, BitmapResourceMixin):
    """
    使用 BitBlt + GetDIBits 的截图实现。
    接口与 PrintWindowScreencapper 保持一致，适合在需要 BitBlt 的场景下使用。
    """
    ...
    def init(self) -> bool:
        ...
        try:
            ...
            if not hwnd:
                raise Exception('未找到目标窗口，无法初始化BitBlt')
            ...
            log.exception("初始化 BitBlt 失败: %s", e)
            self.cleanup()
            return False
